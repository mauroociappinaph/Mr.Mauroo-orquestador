...
Mr-Mauroo-AI exposes a CLI and TUI. It configures MCP for agents. It does not expose a local HTTP API in this repository.

## Interface map

| Interface | Status in this repo | Primary files | Read for details |
|---|---|---|---|
| CLI | Implemented | `cmd/mr-mauroo-ai/main.go`, `internal/app/`, `internal/cli/` | [Usage](../usage.md) |
| TUI | Implemented | `internal/tui/model.go`, `internal/tui/router.go`, `internal/tui/screens/` | [Usage](../usage.md) |
| MCP | Configured, not hosted by Mr-Mauroo-AI | `internal/components/engram/`, `internal/components/mcp/` | [Engram Commands](../engram.md) |
...
Mr-Mauroo-AI writes agent-specific MCP configuration so agents can call external servers. Engram configuration usually invokes:

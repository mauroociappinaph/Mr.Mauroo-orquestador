...
// the OpenCode/Kilocode `mr-mauroo` agent definition in settings JSON, AND
// the Claude Code output-style overlay. Used by `mr-mauroo-ai install`.
...
// InjectForSync regenerates the persona assets that `mr-mauroo-ai sync` is
// allowed to touch. It writes:
//   - The marker-bound persona block in the agent's prompt file (markdown).
//   - The Mr.Mauroo output-style file + outputStyle settings overlay (Claude
//     Code only — no conflict with other components).
//
// It deliberately skips the OpenCode/Kilocode `mr-mauroo` agent definition in
...
func injectInternal(homeDir string, adapter agents.Adapter, persona model.PersonaID, syncManaged bool) (InjectionResult, error) {
	...
	case model.StrategyMarkdownSections:
		...
		// Auto-heal: strip any legacy free-text Mr.Mauroo persona block that was
	...
	case model.StrategyFileReplace:
		...
		// For non-Mr.Mauroo personas (e.g. neutral), the content is just a short
		...
		// append their sections. For Mr.Mauroo, the content is the full persona
	...
	case model.StrategyInstructionsFile:
		...
		// Auto-heal: remove any stale Mr.Mauroo persona content left at the
		...
		// For non-Mr.Mauroo personas, preserve managed sections (same logic
	...
	case model.StrategyJinjaModules:
		...
		case isMrMaurooConversationPersona(persona):
			outputStyleContent = assets.MustRead("kimi/output-style-mr-mauroo.md")
	...
	// Mr.Mauroo overlay creation remains install-only because this overlay shares
	// the "agent" key in opencode.json with SDD's gentle-orchestrator overlay.
	// Non-mr-mauroo sync may still do a narrow cleanup of only agent.mr-mauroo so
	...
	if (adapter.Agent() == model.AgentOpenCode || adapter.Agent() == model.AgentKilocode) && persona != model.PersonaCustom {
		...
		if settingsPath != "" {
			if isMrMaurooConversationPersona(persona) {
				if !syncManaged {
				...
				// Non-mr-mauroo: remove any residual agent.mr-mauroo key left by a
				// previous mr-mauroo install. Only the "mr-mauroo" sub-key is removed
				// from within "agent" — other user-defined agents are preserved.
				removed, err := removeJSONNestedSubKey(settingsPath, "agent", "mr-mauroo")
				if err != nil {
					return InjectionResult{}, fmt.Errorf("clean agent.mr-mauroo from settings: %w", err)
				}
				if removed {
					changed = true
					files = append(files, settingsPath)
	...
	// 3. Mr.Mauroo-only: write output style + merge into settings (if agent supports it).
	if isMrMaurooConversationPersona(persona) && adapter.Agent() != model.AgentOpenClaw && adapter.SupportsOutputStyles() {
		outputStyleDir := adapter.OutputStyleDir(homeDir)
		if outputStyleDir != "" {
			outputStylePath := outputStyleDir + "/mr-mauroo.md"
			outputStyleContent := assets.MustRead("claude/output-style-mr-mauroo.md")
		...
		// Merge "outputStyle": "Mr.Mauroo" into settings.
		settingsPath := adapter.SettingsPath(homeDir)
		if settingsPath != "" {
			settingsResult, err := mergeJSONFile(settingsPath, outputStyleOverlayJSON("Mr.Mauroo"))
			if err != nil {
				return InjectionResult{}, err
			}
	...
	// 3b. Non-mr-mauroo cleanup: remove residual Mr.Mauroo output-style artifacts
	// left by a previous install when the user switches away from the mr-mauroo persona.
	if !isMrMaurooConversationPersona(persona) && adapter.Agent() != model.AgentOpenClaw && adapter.SupportsOutputStyles() {
		outputStyleDir := adapter.OutputStyleDir(homeDir)
		if outputStyleDir != "" {
			outputStylePath := outputStyleDir + "/mr-mauroo.md"
			...
			if err != nil {
				return InjectionResult{}, fmt.Errorf("remove mr-mauroo output style: %w", err)
			}
			if styleRemoved {
				changed = true
				files = append(files, outputStylePath)
		...
		settingsPath := adapter.SettingsPath(homeDir)
		if settingsPath != "" {
			removed, err := removeJSONKeyIfValue(settingsPath, "outputStyle", "Mr.Mauroo")
			if err != nil {
...
// already contains a <!-- mr-mauroo-ai:persona --> section. That is the strongest
...
// match of one of the known persona assets (mr-mauroo or neutral). This handles
...
func shouldStripManagedLegacyPersona(existing string) bool {
	return strings.Contains(existing, "<!-- mr-mauroo-ai:persona -->")
}
...
func personaContent(agent model.AgentID, persona model.PersonaID) string {
	...
	default:
		...
		case model.AgentOpenCode, model.AgentKilocode:
			return assets.MustRead("opencode/persona-mr-mauroo.md")
		case model.AgentKimi:
			return assets.MustRead("kimi/persona-mr-mauroo.md")
		case model.AgentKiroIDE:
			...
			return assets.MustRead("kiro/persona-mr-mauroo.md")
		case model.AgentHermes:
			return assets.MustRead("hermes/persona-mr-mauroo.md")
		default:
			// Generic persona includes Mr.Mauroo personality + skills table + SDD orchestrator.
...
// mr-mauroo-ai managed sections (SDD orchestrator, engram protocol, etc.) and
...
// (empty file, Mr.Mauroo persona, or no managed markers found).
func preserveManagedSections(existing, newPersona string, persona model.PersonaID) (string, bool) {
	if existing == "" || isMrMaurooConversationPersona(persona) {
		return "", false
	}

	idx := strings.Index(existing, "<!-- mr-mauroo-ai:")
	if idx < 0 {
		return "", false
	}
...
// isLegacyUnwrappedPersona reports whether content is a Mr.Mauroo persona
...
// contain stale Mr.Mauroo persona content from older installer versions.
...
// cleanLegacyVSCodePersona removes Mr.Mauroo persona content from any old VS Code
// persona file paths that are no longer written by the current installer.
// Only files that contain clear Mr.Mauroo persona fingerprints are removed —
...
func cleanLegacyVSCodePersona(homeDir string) (bool, error) {
	...
	for _, oldPath := range legacyVSCodePersonaPaths(homeDir) {
		...
		if !isLegacyUnwrappedPersona(string(data)) {
			// File exists but doesn't look like a Mr.Mauroo persona — leave it alone.
			continue
		}

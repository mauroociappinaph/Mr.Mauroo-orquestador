...
func TestFindBinaryInTar(t *testing.T) {
	...
	entries := []struct {
		...
		{"mr-mauroo-ai_1.5.0_darwin_arm64/mr-mauroo-ai", content}, // binary in subdir
	}
	...
	outPath := filepath.Join(t.TempDir(), "mr-mauroo-ai")
	...
	err := downloadBinary(context.Background(), server.URL+"/release.tar.gz", "mr-mauroo-ai", outPath)
	if err != nil {
		t.Fatalf("downloadBinary: %v", err)
	}
...
func TestExpectedChecksumFor(t *testing.T) {
	content := "abc123  mr-mauroo-ai_1.0.0_darwin_arm64.tar.gz\ndef456  mr-mauroo-ai_1.0.0_linux_amd64.tar.gz\n"
	...
	tests := []struct {
	...
	}{
		{
			...
			content:  content,
			filename: "mr-mauroo-ai_1.0.0_darwin_arm64.tar.gz",
			want:     "abc123",
		},
		{
			name:     "found second entry",
			content:  content,
			filename: "mr-mauroo-ai_1.0.0_linux_amd64.tar.gz",
			want:     "def456",
		},
		{
			name:     "not found returns error",
			content:  content,
			filename: "mr-mauroo-ai_1.0.0_windows_amd64.zip",
			wantErr:  true,
		},
		{
...
func TestFetchChecksums(t *testing.T) {
	const fakeContent = "abc123  mr-mauroo-ai_1.0.0_darwin_arm64.tar.gz\n"

...
func TestConfigPathsCrossPlatform(t *testing.T) {
	...
	if got := a.SystemPromptFile(home); got != filepath.Join(home, ".cursor", "rules", "mr-mauroo-ai.mdc") {
		t.Fatalf("SystemPromptFile() = %q, want %q", got, filepath.Join(home, ".cursor", "rules", "mr-mauroo-ai.mdc"))
	}
}

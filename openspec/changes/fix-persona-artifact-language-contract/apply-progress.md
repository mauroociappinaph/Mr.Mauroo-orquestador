...
- Added `mr-mauroo-neutral-artifacts` model, CLI validation, TUI option, review label, and persona injection support.
...
- Preserved Mr.Mauroo direct-conversation voice while adding explicit artifact/comment boundaries to persona assets.
...
- `internal/assets/*/persona-mr-mauroo.md`
...
| `mr-mauroo-neutral-artifacts` support | `internal/cli/persona_language_contract_test.go`, `internal/tui/screens/persona_language_contract_test.go`, `internal/components/persona/persona_language_contract_test.go` | Unit/component | N/A new tests | Compile failed because `PersonaMr.MaurooNeutralArtifacts` was undefined | Passed after model, CLI, TUI, and persona injection support | Covered CLI normalization, TUI rendering, and OpenCode persona injection | `gofmt` |
...
- `go test ./internal/assets/... -run 'TestSDDOrchestratorAssetsEnforceLanguageContract|TestSupportedAgentSDDLanguageMatrix|TestCommentWriterLanguageContractSources|TestMr.MaurooPersonaKeepsDirectConversationVoice'` — RED failed as expected.
...
- `go test ./internal/cli/... -run TestNormalizePersonaAcceptsMr.MaurooNeutralArtifacts` — GREEN passed after implementation.
- `go test ./internal/tui/screens/... -run 'TestPersonaOptionsIncludeMr.MaurooNeutralArtifacts|TestRenderPersonaDescribesMr.MaurooNeutralArtifacts'` — GREEN passed after implementation.
- `go test ./internal/components/persona/... -run TestInjectMr.MaurooNeutralArtifactsUsesMr.MaurooConversationWithArtifactBoundary` — GREEN passed after implementation.
...
- Implemented `mr-mauroo-neutral-artifacts` support because discovery confirmed it was missing and tasks marked it conditional in scope.

...
Neutral should provide Mr.Mauroo-equivalent mentor behavior without Rioplatense/regional voice. Today neutral avoids most direct Mr.Mauroo voice leakage, but has a weaker behavior contract and legacy sync can reactivate Mr.Mauroo when persona state is missing or invalid.
...
- Fix sync defaults so missing/invalid persisted persona does not silently fall back to Mr.Mauroo.

### Out of Scope
- Changing Mr.Mauroo regional voice or mentor contract.
- Reworking OpenCode/Kilocode residual `agent.mr-mauroo` behavior unless needed to prevent neutral regressions.
...
| OpenCode/Kilocode sync assets | Investigate | Confirm residual `agent.mr-mauroo` behavior is intentional and not a neutral regression. |

## Risks

| Risk | Likelihood | Mitigation |
|------|------------|------------|
| Neutral gains regional/Mr.Mauroo voice | Medium | Add neutral voice constraints and regression coverage. |
| Legacy users lose expected Mr.Mauroo default | Medium | Define fallback semantics clearly; preserve explicit Mr.Mauroo selections. |
...
- [ ] Neutral receives Mr.Mauroo-equivalent behavior rules without regional voice.
- [ ] Claude neutral has a non-default output-style contract.
- [ ] Kimi neutral injected `output-style.md` is not empty.
- [ ] Missing/invalid persisted persona no longer silently reactivates Mr.Mauroo.
- [ ] Explicit Mr.Mauroo selection continues to work.
